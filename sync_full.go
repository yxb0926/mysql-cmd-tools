...
func Producer(item chan []string) {
	...
	//
	minmaxPKMap := MinMaxPK(tx)
	SourceValues(minmaxPKMap["minpk"], minmaxPKMap["maxpk"], item, tx)
}
...
func SourceValues(minpk int64, maxpk int64, item chan []string, tx *sql.Tx) {
	...
	for i := minpk; i <= maxpk; i = i + dsnfromtbinfo.offset {
		if (i + dsnfromtbinfo.offset) < maxpk {
			...
			fmt.Println("-- Get Data From Source Table Complete.")
...
func MinMaxPK(tx *sql.Tx) (maxminpkMap map[string]int64) {
